...
var rPassAccuracy=function(playerId){
  console.log(playerId);
  var groundPass=0;
  var aerialPass=0;
  var unsuccess=0;
  k.forEach(function(index,i){
    if(index.playerNumber==playerId){
      if(index.action=='R Grounded Pass')
      {
        groundPass++;
      }
      if(index.action=='R Aerial Pass')
      {
        aerialPass++;
      }
      if(index.action=='R Unsucc Pass')
      {
        unsuccess++;
      }
  ...
  console.log("DSf",((groundPass+aerialPass)/(groundPass+aerialPass+unsuccess))*100)
  return ((groundPass+aerialPass)/(groundPass+aerialPass+unsuccess))*100;
}
var rPassAccuracyTeam=function(playerTeam){
  console.log(playerTeam);
  var groundPass=0;
  var aerialPass=0;
  var unsuccess=0;
  k.forEach(function(index,i){
    if(index.playerTeam==playerTeam){
      if(index.action=='R Grounded Pass')
      {
        groundPass++;
      }
  ...
  console.log("DSfAD",((groundPass+aerialPass)/(groundPass+aerialPass+unsuccess))*100)
  return ((groundPass+aerialPass)/(groundPass+aerialPass+unsuccess))*100;
}

var lPassAccuracy=function(playerId){
  console.log(playerId);
  var groundPass=0;
  var aerialPass=0;
  var unsuccess=0;
  k.forEach(function(index,i){
    if(index.playerNumber==playerId){
      if(index.action=='L Grounded Pass')
      {
        groundPass++;
      }
      if(index.action=='L Aerial Pass')
      {
        aerialPass++;
      }
      if(index.action=='L Unsucc Pass')
      {
        unsuccess++;
      }
...
var lPassAccuracyTeam=function(playerTeam){
  console.log(playerTeam);
  var groundPass=0;
  var aerialPass=0;
  var unsuccess=0;
...
var lShotAccuracy=function(playerId){
  console.log(playerId);
  var shotOn=0;
  var goal=0;
  var shotPost=0;
  ...
  k.forEach(function(index,i){
    if(index.playerNumber==playerId){
      if(index.action=='L Shot On')
      {
        shotOn++;
      }
      if(index.action=='L Goal' || index.action=='Goal Kick')
      {
        goal++;
      }
      if(index.action=='L Shot Wide')
      {
        shotWide++;
      }
      if(index.action=='L Shot Post'){
        shotPost++;
      }
      
    }
  })
  console.log("DSf",playerId,shotOn,goal,shotWide,shotPost)
  return ((shotOn+goal)/(shotOn+goal+shotWide+shotPost))*100;
}
var lShotAccuracyTeam=function(playerTeam){
  console.log(playerTeam);
  var shotOn=0;
  var goal=0;
  var shotPost=0;
  ...
  console.log("DSfAD",((shotOn+goal)/(shotOn+goal+shotWide+shotPost))*100)
  return ((shotOn+goal)/(shotOn+goal+shotWide+shotPost))*100;
}

var rShotAccuracy=function(playerId){
  console.log(playerId);
  var shotOn=0;
  var goal=0;
  var shotPost=0;
  ...
  k.forEach(function(index,i){
    if(index.playerNumber==playerId){
      if(index.action=='R Shot On')
      {
        shotOn++;
      }
      if(index.action=='R Goal' || index.action=='Goal Kick')
      {
        goal++;
      }
      if(index.action=='R Shot Wide')
      {
        shotWide++;
      }
      if(index.action=='R Shot Post'){
        shotPost++;
      }
...
var rShotAccuracyTeam=function(playerTeam){
  console.log(playerTeam);
  var shotOn=0;
  var goal=0;
  var shotPost=0;
...
var config = {
  ...
  data: {
    labels: [
      "B1", "B2", "B3","B4","B5","Team B"],
    datasets: [{
      label: 'R pass Accuracy',
        ...
        rShotAccuracyTeam("G")
          ...
          "B"
			...
			options: {
				...
				title: {
					display: true,
					text: 'R Shot Accuracy for B team'
				},
				scale: {
					ticks: {
						beginAtZero: true
...
window.onload = function() {
  ...
  window.myRadar1 = new Chart(document.getElementById("canvas1"), config1);
  window.polar=new Chart.PolarArea(document.getElementById("canvas2"), config3)
  
  
  myRadar.options.elements.line.tension = 0.05;
			myRadar.update();
  myRadar1.options.elements.line.tension = 0.05;
			myRadar1.update();
};
